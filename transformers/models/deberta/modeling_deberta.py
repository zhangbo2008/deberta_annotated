...
class XSoftmax(torch.autograd.Function):
    ...
    def forward(self, input, mask, dim):
        ...
        self.save_for_backward(output) # save的东西用来求导.
        return output

    @staticmethod
    def backward(self, grad_output):
        (output,) = self.saved_tensors # 求导先读取结果.
...
def get_mask(input, local_context):
    ...
    if dropout > 0 and mask is None:
        # dropout 0.1概率丢弃,那么1-dropout 是0.9.bernoulli_之后是0.9概率取1.
        #1-之后就是0.9概率取0. 0.1概率取1.而mask是按照1进行遮罩的.所以0.1 概率丢弃.
...
# https://blog.csdn.net/nini_coded/article/details/79302800  官方实现dropout
#下面这个dropout版本.会比官方更好,因为没有使用乘法而是用的mask,加速了算法!!!!!!
class XDropout(torch.autograd.Function):
    """Optimized dropout function to save computation and memory by using mask operation instead of multiplication."""
# pytorch自定义函数需要重写forward 和backwod函数.
    @staticmethod
    def forward(ctx, input, local_ctx):
        mask, dropout = get_mask(input, local_ctx)
        ctx.scale = 1.0 / (1 - dropout) # 这个放缩保证了dropout之后跟之前也一样的sum值.
        if dropout > 0:
            ctx.save_for_backward(mask) # 保存这个mask变量到ctx里面.
            return input.masked_fill(mask, 0) * ctx.scale # 对input这个tensor进行mask,mask里面是1的部分进行0填充.
        else:
            return input
...
#实现dropout层.对上面XDropout 进行了封装.把函数包装成了module.也就是网络layer
...
class DebertaLayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        mean = hidden_states.mean(-1, keepdim=True)     # -1表示按最后一列求平均值. 也就是压缩这一个维度.所以结果就是 (batch,seq_len,1)  layernorm就是对channel做归一化.
        variance = (hidden_states - mean).pow(2).mean(-1, keepdim=True)
        hidden_states = (hidden_states - mean) / torch.sqrt(variance + self.variance_epsilon)
        hidden_states = hidden_states.to(input_type)
        y = self.weight * hidden_states + self.bias  # 归一化最后再变回来.这样bp之后梯度都进入了这个self.weight和self.bias里面.对于下一层而言.输出的hidden_states就是归一化之后的.
        return y

# 原本的输出层. 就是linear 再dropout.加 LN层
...
class DebertaLayer(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.attention(     # 注意力计算在这里面!核心部分.
            hidden_states,
            attention_mask,
...
class DebertaEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.relative_attention:
            ...
            if self.max_relative_positions < 1: # 读取相关注意力的最大positions参数.
    ...
    def get_attention_mask(self, attention_mask):
        if attention_mask.dim() <= 2:
            ...
            attention_mask = attention_mask.byte() # 变成int8来存储
    ...
    def get_rel_pos(self, hidden_states, query_states=None, relative_pos=None):
        if self.relative_attention and relative_pos is None:
            q = query_states.size(-2) if query_states is not None else hidden_states.size(-2) # q是序列长度
    ...
    def forward(
        ...
        attention_mask = self.get_attention_mask(attention_mask) # 计算相对位置
        ...
        rel_embeddings = self.get_rel_embedding() # 随机生成一个1024*768矩阵
...
def build_relative_position(query_size, key_size, device):
    ...
    rel_pos_ids = q_ids[:, None] - k_ids.view(1, -1).repeat(query_size, 1) # 矩阵横减去竖的索引组成的矩阵.
...
#加上bias
        ...
        scale_factor = 1 + len(self.pos_att_type) # 一个是content2position 一个是position2content
...
# context_layer 表示输出的特征.
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if "c2p" in self.pos_att_type:
            ...
            c2p_pos = torch.clamp(relative_pos + att_span, 0, att_span * 2 - 1) # 加到大于0.因为索引是没有小于0的.所以我们加上偏执项才可以用上下面的gather功能.
...
# 全局position_bias
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, mask=None, inputs_embeds=None):
        ...
        if self.position_embeddings is not None:
            position_embeddings = self.position_embeddings(position_ids.long()) # 这里面是绝对位置的embedding.
        else:
            position_embeddings = torch.zeros_like(inputs_embeds)

        embeddings = inputs_embeds
        if self.position_biased_input: # 是否计算positon的bias. 这里面是false
...
class DebertaModel(DebertaPreTrainedModel):
    ...
    def forward(
        ...
        if self.z_steps > 1:# z_表示额外再计算几遍encoding
